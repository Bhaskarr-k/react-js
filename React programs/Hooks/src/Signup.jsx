...
export default function Signup() {
    // state variables to store user inputs i nthe form of object
    ...
    // storing user input data into state by usijg onChange listener and handlingchange
    ...
    // Regular expreesion for validation
    const regex={
        un:/^[A-Z][a-z]{3,8}$/,
        pw:/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[!@#$%^&*]).{6,16}$/,
    ...
    function validate(){
        ...
        if(!regex.un.test(formData.username))
          erors.username="username should start with capital letter followed by small letters"  
        if(!regex.pw.test(formData.password))
            erors.password="password must contain atleast 1 capital 1 small 1 digit and 1 special charecter with length of 6 to 16"
